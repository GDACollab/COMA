...
public class contactC : MonoBehaviour {
	...
	void Update () {
		if (Input.GetKeyDown (KeyCode.Semicolon)) {
			if ( missed == 0){
				if (Vector3.Distance(this.transform.position, source.Cscore[source.top].transform.position) < .16){
					source.kill ();
					words.enabled = true;
					words.text = "Good";
				}else if (Vector3.Distance(this.transform.position, source.Cscore[source.top].transform.position)<.33){
					source.kill ();
					words.enabled = true;
					words.text = "Bad";
					//lower health
